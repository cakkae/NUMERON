...
export default function HomePage() {
  ...
  return <main className="card"><header><div><h1>{activeCompany.name}</h1><p>PDV: {activeCompany.vat_number}</p></div><label>Aktivna firma<select value={activeCompanyId} onChange={(event) => setActiveCompanyId(event.target.value)}>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label></header><section><h2>Pregled</h2><p>Odabrana firma je aktivna. KUF/KIF moduli još nisu dio ove faze.</p></section></main>;
}
