...
import { Toast, WorkspaceSkeleton } from "@/components/ui/primitives";
...
export default function HomePage() {
  ...
  if (workspace.loading) return <WorkspaceSkeleton />;
  ...
  const content = view === "dashboard" ? <DashboardView company={workspace.activeCompany} period={workspace.activePeriod} purchaseCount={workspace.purchases.length} salesCount={workspace.sales.length} errorCount={workspace.validationStatus.errorCount} errors={workspace.validationStatus.errors} onNavigate={setView} onCreatePeriod={() => setPeriodDialogOpen(true)} onReady={() => void workspace.setPeriodStatus("ready_for_export")} onTogglePeriod={() => void workspace.setPeriodStatus(workspace.activePeriod?.status === "open" ? "locked" : "open")} /> : view === "kuf" ? <BookView title="KUF" entries={workspace.purchases} partners={workspace.partners} periodIsOpen={workspace.periodIsOpen} periodLabel={periodLabel} errors={workspace.kufErrors} onSave={(form, id) => workspace.saveEntry("purchase_entries", form, id)} onMutate={(id, action) => void workspace.mutateEntry("purchase_entries", id, action)} /> : view === "kif" ? <BookView title="KIF" entries={workspace.sales} partners={workspace.partners} periodIsOpen={workspace.periodIsOpen} periodLabel={periodLabel} errors={workspace.kifErrors} onSave={(form, id) => workspace.saveEntry("sales_entries", form, id)} onMutate={(id, action) => void workspace.mutateEntry("sales_entries", id, action)} /> : view === "partners" ? <PartnersView partners={workspace.partners} onCreate={workspace.createPartner} /> : view === "documents" ? <DocumentsView company={workspace.activeCompany} periods={workspace.periods} activePeriodId={workspace.activePeriodId} partners={workspace.partners} purchases={workspace.purchases} sales={workspace.sales} documents={workspace.documents} onRefresh={() => workspace.refreshDocuments(workspace.activeCompany!.id)} onPosted={workspace.refreshAfterDocumentPosting} onMessage={workspace.setMessage} /> : view === "exports" ? <ExportsView company={workspace.activeCompany} period={workspace.activePeriod} purchases={workspace.purchases} sales={workspace.sales} archives={workspace.exportArchives} onRefresh={workspace.refreshExports} onMessage={workspace.setMessage} /> : <div className="panel placeholder-panel"><h2>Postavke</h2><p>Ovaj modul nije dio Faze 5.5.</p></div>;

  return <><AppShell view={view} onNavigate={setView} companies={workspace.companies} activeCompanyId={workspace.activeCompanyId} onCompanyChange={workspace.setActiveCompanyId} periods={workspace.periods} activePeriodId={workspace.activePeriodId} onPeriodChange={workspace.setActivePeriodId} onCreatePeriod={() => setPeriodDialogOpen(true)} activePeriod={workspace.activePeriod} userEmail={workspace.userEmail} onSignOut={() => void workspace.signOut()}>{content}</AppShell><PeriodDialog open={periodDialogOpen} onClose={() => setPeriodDialogOpen(false)} onCreate={workspace.createPeriod} />{workspace.message && <Toast message={workspace.message} onClose={() => workspace.setMessage("")} />}</>;
}
