...
create type public.workspace_role as enum ('owner', 'client');
create type public.company_role as enum ('owner', 'client');
...
create function public.can_edit_company(target_company_id uuid) returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.company_members where company_id = target_company_id and user_id = auth.uid() and role = 'owner');
$$;
...
create policy "company owners update assigned companies" on public.companies for update using (public.can_edit_company(id)) with check (public.can_edit_company(id));
